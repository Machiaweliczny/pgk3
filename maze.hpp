...
public:
	...
	cube * nearbyCube(glm::vec3 place){
		for (auto i = cubes.begin(); i != cubes.end(); i++){
			if ((*i).type == Maze::TRACK) continue;
			float zdiff = (*i).pos.z - place.z;
			float xdiff = (*i).pos.x - place.x;
			float distance = sqrt(zdiff*zdiff + xdiff*xdiff);
